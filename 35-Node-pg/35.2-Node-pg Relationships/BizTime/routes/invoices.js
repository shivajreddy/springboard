...
invoices.get('/:id', async(req, res, next)=>{
  try {
    ...
    const comp_query = await db.query(
      `SELECT code, name, description
      FROM companies
      WHERE code=$1`,
      [result.rows[0].comp_code]
    )
    if(comp_query.rows.length === 0){return res.status(200).send({ invoice: result.rows[0], company: "no company info found"}) };
...
invoices.put('/:id', async(req, res, next)=>{
  ...
  */
  try {
    ...
    if(comp_query.rows.length === 0){throw new ExpressError(`Given ${comp_code} is not there in company table`)};
    ...
    if(invoice_query.rows.length === 0){throw new ExpressError(`No invoice found for id${id}`, 404)};

    // to make a payment, paid_date should be false, and paid should be true
    // i.o.w check if paid is being true, and there is no paid_date data already
    const currPaidDate = invoice_query.rows[0].paid_date;
    let paidDate = null
    paidDate = (!currPaidDate && paid) ? new Date() : currPaidDate;
